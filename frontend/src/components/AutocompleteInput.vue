<template>
  <section class="section">
    ...
    <div class="autocomplete-wrap" v-if="modelValue.length < limit" ref="wrapRef">
      <div class="input-row">
        <div class="input-wrapper" :class="{ loading: searching }">
          <input
            v-model="query"
            type="text"
...
const props = defineProps({
  ...
  // Para person: 'Acting' | 'Directing' | null (ambos)
...
async function search(q) {
  ...
  try {
    if (props.type === 'movie') {
      ...
      if (props.department) {
        results = results.filter(p => p.known_for_department === props.department)
      }
...
function select(item) {
  ...
  const already = props.modelValue.some(i => i.id === item.id)
  if (!already) {
...
function selectFirst() {
  if (suggestions.value.length) select(suggestions.value[highlighted.value])
}
...
.limit-badge {
  ...
  border-radius: 50px; padding: 0.2rem 0.6rem; white-space: nowrap; flex-shrink: 0; transition: all 0.2s;
}
...
.tag {
  ...
  border-radius: 50px; color: #d4af37; font-size: 0.82rem; font-family: 'DM Sans', sans-serif;
...
.tag button {
  background: none; border: none; cursor: pointer; color: #6b6050;
  font-size: 1.1rem; line-height: 1; padding: 0; display: flex;
...
.input-wrapper input {
  width: 100%; padding: 0.6rem 2.25rem 0.6rem 0.875rem;
  background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08);
  border-radius: 8px; color: #e8e0d0; font-family: 'DM Sans', sans-serif;
...
.spinner-mini {
  ...
  width: 14px; height: 14px; border: 2px solid rgba(212,175,55,0.15);
  border-top-color: #d4af37; border-radius: 50%; animation: spin 0.7s linear infinite;
}
...
.suggestions {
  ...
  background: #0f0f15; border: 1px solid rgba(212,175,55,0.2);
  border-radius: 12px; overflow: hidden;
  box-shadow: 0 16px 48px rgba(0,0,0,0.7); z-index: 300; max-height: 280px; overflow-y: auto;
}

.suggestion-item {
  display: flex; align-items: center; gap: 0.75rem; width: 100%;
  padding: 0.65rem 0.875rem; background: none; border: none; cursor: pointer;
  text-align: left; transition: background 0.12s; border-bottom: 1px solid rgba(255,255,255,0.03);
}
...
.suggestion-img-placeholder {
  width: 36px; height: 36px; border-radius: 6px; background: rgba(212,175,55,0.1);
  display: flex; align-items: center; justify-content: center;
  color: #d4af37; font-size: 0.9rem; font-weight: 600; flex-shrink: 0;
}
